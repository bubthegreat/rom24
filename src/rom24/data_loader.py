...
def load_npcs(area, pArea):
    ...
    while w != "0":
        ...
        npc.start_pos = state_checks.name_lookup(
            tables.position_table, npc.start_pos, "short_name"
        )
        npc.default_pos = state_checks.name_lookup(
            tables.position_table, npc.default_pos, "short_name"
        )
...
def load_objects(area, pArea):
    ...
    while w != "0":
        ...
        if item.item_type == merc.ITEM_WEAPON:
            ...
            item.item_type == merc.ITEM_DRINK_CON
            ...
            item.item_type == merc.ITEM_POTION
            or item.item_type == merc.ITEM_SCROLL
...
def load_resets(area, pArea):
    ...
    while True:
        ...
        area, reset.arg3 = (
            (area, 0) if letter == "G" or letter == "R" else game_utils.read_int(area)
        )
        area, reset.arg4 = (
            game_utils.read_int(area) if letter == "P" or letter == "M" else (area, 0)
        )
        area, t = game_utils.read_to_eol(area)
