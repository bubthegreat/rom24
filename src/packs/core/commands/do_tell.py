...
def do_tell(ctx):
    ...
    if not (
        ch.is_immortal() and ch.level > merc.LEVEL_IMMORTAL
    ) and not state_checks.IS_AWAKE(victim):
        handler_game.act("$E can't hear you.", ch, 0, victim, merc.TO_CHAR)
        return

    if (
        victim.comm.is_set(merc.COMM_QUIET)
        or state_checks.IS_SET(victim.comm, merc.COMM_DEAF)
    ...
    if victim.comm.is_set(merc.COMM_AFK):
        if victim.is_npc():
            handler_game.act(
                "$E is AFK, and not receiving tells.", ch, None, victim, merc.TO_CHAR
            )
            return
        handler_game.act(
