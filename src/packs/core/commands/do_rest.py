...
def do_rest(ctx):
    ...
    if argument:
        ...
        if obj:
            if obj.item_type != merc.ITEM_FURNITURE or (
                not state_checks.IS_SET(obj.value[2], merc.REST_ON)
    ...
    if ch.position == merc.POS_SLEEPING:
        ...
        if not obj:
            ...
            handler_game.act(
                "$n wakes up and starts resting.", ch, None, None, merc.TO_ROOM
            )
        elif state_checks.IS_SET(obj.value[2], merc.REST_AT):
            handler_game.act(
                "You wake up and rest at $p.",
                ch,
                obj,
                None,
                merc.TO_CHAR,
            ...
            handler_game.act(
                "$n wakes up and rests at $p.", ch, obj, None, merc.TO_ROOM
            )
        elif state_checks.IS_SET(obj.value[2], merc.REST_ON):
            handler_game.act(
                "You wake up and rest on $p.",
                ch,
                obj,
                None,
                merc.TO_CHAR,
            ...
            handler_game.act(
                "$n wakes up and rests on $p.", ch, obj, None, merc.TO_ROOM
            )
        else:
            handler_game.act(
                "You wake up and rest in $p.",
                ch,
                obj,
                None,
                merc.TO_CHAR,
            ...
            handler_game.act(
                "$n wakes up and rests in $p.", ch, obj, None, merc.TO_ROOM
            )
        ...
        if obj is None:
            ...
            handler_game.act(
                "You sit down at $p and rest.", ch, obj, None, merc.TO_CHAR
            )
            handler_game.act(
                "$n sits down at $p and rests.", ch, obj, None, merc.TO_ROOM
            )
        elif state_checks.IS_SET(obj.value[2], merc.REST_ON):
...
api.register("rest", do_rest, pos=merc.POS_SLEEPING, level=0, log=merc.LOG_NORMAL, show=1)
