...
def do_reply(ctx):
    ...
    if not victim.desc and not victim.is_npc():
        handler_game.act(
            "$N seems to have misplaced $S link...try again later.",
            ch,
            None,
            victim,
            merc.TO_CHAR,
    ...
    if (
        (victim.comm.is_set(merc.COMM_QUIET) or victim.comm.is_set(merc.COMM_DEAF))
        and not ch.is_immortal()
        and not state_checks.IS_IMMORTAL(victim)
    ):
        handler_game.act(
            "$E is not receiving tells.", ch, None, victim, merc.TO_CHAR, merc.POS_DEAD
        )
        return
    ...
    if victim.comm.is_set(merc.COMM_AFK):
        if victim.is_npc():
            handler_game.act(
                "$E is AFK, and not receiving tells.",
                ch,
                None,
                victim,
                merc.TO_CHAR,
        ...
        return
    handler_game.act(
        "You tell $N '$t'", ch, argument, victim, merc.TO_CHAR, merc.POS_DEAD
    )
    handler_game.act(
        "$n tells you '$t'", ch, argument, victim, merc.TO_VICT, merc.POS_DEAD
    )
    victim.reply = ch
    return
