...
def do_clone(ctx):
    ...
    if obj:
        ...
        handler_game.wiznet(
            "$N clones $p.", ch, clone, merc.WIZ_LOAD, merc.WIZ_SECURE, ch.trust
        )
        return
    elif mob:
        if not state_checks.IS_NPC(mob):
            ...
            return
        if (
            (mob.level > 20 and not state_checks.IS_TRUSTED(ch, merc.L4))
...
api.register("clone", do_clone, pos=merc.POS_DEAD, level=merc.L5, log=merc.LOG_ALWAYS, show=1)
