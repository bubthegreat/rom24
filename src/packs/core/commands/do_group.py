...
def do_group(ctx):
    ...
    if victim.master != ch and ch != victim:
        handler_game.act(
            "$N isn't following you.", ch, None, victim, merc.TO_CHAR, merc.POS_SLEEPING
        )
        return
    ...
    if ch.is_affected(merc.AFF_CHARM):
        handler_game.act(
            "You like your master too much to leave $m!",
            ch,
            None,
            victim,
            merc.TO_VICT,
    ...
    victim.leader = ch
    handler_game.act(
        "$N joins $n's group.", ch, None, victim, merc.TO_NOTVICT, merc.POS_RESTING
    )
    handler_game.act(
        "You join $n's group.", ch, None, victim, merc.TO_VICT, merc.POS_SLEEPING
    )
    handler_game.act(
        "$N joins your group.", ch, None, victim, merc.TO_CHAR, merc.POS_SLEEPING
    )
    return
