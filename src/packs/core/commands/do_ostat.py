...
def do_ostat(ctx):
    ...
    ch.send("Name(s): %s\n" % obj.name)
    ch.send(
        "Vnum: %d  Format: %s  Type: %s  Resets: %d\n"
        % (obj.vnum, "new" if obj.new_format else "old", obj.item_type, obj.reset_num)
    )
    ch.send(
        "Short description: %s\nLong description: %s\n"
    ...
    ch.send(
        "Level: %d  Cost: %d  Condition: %d  Timer: %d\n"
    ...
    ch.send(
        ...
        % (
            ...
            "(noone)"
            if not obj.in_living
            else "someone"
            if not ch.can_see(obj.in_living)
            else obj.in_living.name,
        )
    )
    ...
    if (
        obj.item_type == merc.ITEM_SCROLL
        ...
        ch.send(
            ...
            % (const.liq_table[obj.value[2]].color, const.liq_table[obj.value[2]].name)
        )
        ...
        ch.send(
            ...
            % (
                ...
                if obj.value[3] in const.attack_table
                else "undefined"
            )
        )
